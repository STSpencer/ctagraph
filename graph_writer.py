...
runno = 50
gamma_data = "/store/muonsims/gammas/run" + str(runno) + ".simtel.gz"
hadron_data = "/store/muonsims/protons/run"+str(runno)+".simtel.gz"
...
source = event_source('/store/muonsims/muons/run50.simtel.gz')
...
for fileno in np.arange(1, no_files + 1):
    ...
    raw_group = h5file.create_dataset(
        "raw_images", (no_events, 4, 48*48), dtype='f', compression="gzip")
